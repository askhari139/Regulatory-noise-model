...
\end{center}
\begin{abstract}
Gene regulatory network models treat interaction parameters as fixed, although regulatory efficacy fluctuates with molecular context. We asked how temporal fluctuations in interaction strength reshape phenotype occupancy in cell-fate decision GRN motifs. Across large ensembles of parameter sets, fluctuations anchored to initial values largely preserved deterministic state occupancies. Additive fluctuations in interaction strengths increased occupancy of all-high co-expression states, particularly in multistable regimes where high expression was accessible. In contrast, multiplicative fluctuations biased inhibitory interactions toward stronger repression and favored single-high states in a topology-dependent manner. Control deterministic simulations sampled from the noise-induced parameter distributions did not fully reproduce these effects. A Boolean-limit analysis attributed the additional additive bias to an asymmetry between unconditional derepression and conditional repression. Analyses of epithelial–mesenchymal plasticity and gonadal-fate networks further demonstrated increased occupancy of hybrid team-expression states under additive fluctuations. Thus, regulatory noise can reshape the developmental landscape in opposing directions, pushing cell-fate systems toward either progenitor-like or terminally differentiated states.
...
\begin{figure}
    ...
    \caption{\textbf{Effect of multiplicative noise on the phenotypic landscape of cell-fate decision networks.}
...
\begin{figure}
    ...
    \caption{\textbf{Fate resolution of progenitor state under multiplicative noise}
    ...
    \textbf{(D)} Circular chord diagram of double-high $\to$ single-high MRT flow among monostable double-high parameter sets, at $\sigma$ = 0.01.
...
\begin{figure}
    ...
    \caption{\textbf{Effect of regulatory noise on large biological networks} \textbf{(A)} Representative trajectories of the phenotypic score for EMT network (i) and Gonadal network (ii) at additive noise of 0.05 magnitude. \textbf{(B)} Density of team scores across the representative trajectory of the EMT network, with frequency represented on a scale from low (blue) to high (yellow). WT represents the frequency of team score configurations obtained from the steady states of the unperturbed network, while Rand represents that of the simulations corresponding to the trajectories in A. \textbf{(C)} Relative frequency (equivalent to MRT) of different phenotypic scores across 1000 simulation trajectories of additive noise over simulation time for (i) EMT network, (ii) Gonadal network. \textbf{(D)} Same as C, but for Multiplicative and Stationary noise for EMT network.}
    \label{fig:5}
\end{figure}
